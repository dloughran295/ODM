...
dist = 275*1609; % distance [m] (25 miles)
...
% % % % % % % % % % % % % % % % % % % % % % % % % % 
        ...
        while cond1 == 1
            ...
            s_wing = (x_wing(i) * Wg)/(0.5 * rho * Vfwd^2 * sqrt(pi * AR_wing * e_wing * Cdo_aircraft)); % wing area [m^2](equation from page 136 raymer)
            b_wing = sqrt(AR_wing * s_wing); % wing span [m]
            Cl_wing = x_wing(i)*Wg/(0.5*rho*Vfwd^2*s_wing); % lift coefficient of the wing
...
%                 % converged, exit the outer loop
            ...
            if powerCalc == 1
                ...
                T = sqrt((Wg_new - x_wing(i)*Wg_new)^2 + D^2); % thrust updated with percent weight offload
...
%         hoverpowers(i,j) = Ptotal_hover/1000;
        ...
        energies(i) = Ec_tot/1000;
        weights(i) = Wg_new * 0.2247;
        radii(i) = R*3.28;
        hoverpowers(i)= Ptotal_hover/1000;
...
figure(1)
set(gcf, 'color', 'w')
subplot(2,2,4), plot(x_wing,energies, 'k')
title('400 Wh/kg, 1000 s, 275 miles');
xlabel('Weight Offset Factor')
ylabel('Energy (kWh)')
% subplot(2,2,2), plot(x_wing, weights, 'k')
% xlabel('Weight Offset Factor')
% ylabel('Weight (lbs)')
% subplot(2,2,3), plot(x_wing, radii, 'k')
% xlabel('Weight Offset Factor')
% ylabel('Radius (ft)')
% subplot(2,2,4), plot(x_wing, hoverpowers, 'k')
% xlabel('Weight Offset Factor')
% ylabel('Hover Power (kW)')
%leg = legend('144 Wh/kg, 240 s, 40 miles', '144 Wh/kg, 500 s, 40 miles','Location', 'NW');
%leg = legend('400 Wh/kg, 240 s, 300 miles', '400 Wh/kg, 1000 s, 275 miles','Location', 'NW');
%leg.FontSize = 10;
